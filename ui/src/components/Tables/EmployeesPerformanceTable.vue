<template>
  <v-data-table :headers="headers" :items="employees" class="text-caption text-center" density="compact"
    item-value="name" hide-default-footer hover>
    <template v-slot:item="{ item }">
      <tr class="text-no-wrap text-center">
        ...
        <td>{{ item.status }}</td>
        <td class="text-end">
          <v-chip :text="item.score.toFixed(2)" append-icon="mdi-open-in-new" size="x-small" @click=""></v-chip>
        </td>
      </tr>
    </template>
  </v-data-table>
